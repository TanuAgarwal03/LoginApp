...
class PollDetailPage extends StatefulWidget {
  const PollDetailPage({
    ...
    required this.poll,
    required this.refreshPollList,
  }) : super(key: key);

  final String token;
  final String userId;
  final Map<String, dynamic> poll;
  final VoidCallback refreshPollList;
...
class _PollDetailPageState extends State<PollDetailPage> {
  ...
  Future<void> submitAnswer(String optionId) async {
    ...
    final response = await apiService.postAPI('polls/answer/',headers: {
    ...
    if (response.statusCode == 201) {
      ...
      // print('Successfully voted for $selectedOptionTitle');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Successfully voted for $selectedOptionTitle'),
          duration: const Duration(seconds: 3),
          backgroundColor: Colors.green,
          elevation: 15.0,
        ),
      );
      widget.refreshPollList();
    } else {
      setState(() {
        _isLoading = false;
      });
