...
class _PostDetailPageState extends State<PostDetailPage> {
  ...
  Future<void> _fetchPostDetail() async {
    ...
    try {
      final response = await apiService.getAPI('blogs/posts/${widget.slug}/');
  ...
  Future<void> _toggleLike() async {
    ...
    try {
      ...
      final url = userExists ? 'blogs/action/$actionId/' : 'blogs/action/';
  ...
  Future<void> _fetchCategories() async {
    ...
    // );
    final response = await apiService.getAPI('blogs/categories/');
    if (response.statusCode == 200) {
  ...
  Future<void> _fetchTags() async {
    final response = await apiService.getAPI('blogs/tags/');
    // SharedPreferences prefs = await SharedPreferences.getInstance();
    // token = prefs.getString('token') ?? '';
    // final response = await http.get(
        ...
        if (isTranslated) {
          ...
          for (var comment in comments) {
            var translatedComment =
                await translator.translate(comment['content'], to: 'hi');
            comment['content'] = translatedComment.text;

            for (var reply in comment['replies']) {
              var translatedReply =
                  await translator.translate(reply['content'], to: 'hi');
                                  ...
                                  showModalBottomSheet(
                                      ...
                                      builder: (context) {
                                        return StatefulBuilder(builder:
                                            (BuildContext builder,StateSetter setState) {
                                          return DraggableScrollableSheet(
                                            expand: false,
                                            initialChildSize: 0.5, // Initial size of the sheet as a fraction of the screen height
                                            ...
                                            child: Column(
                                              children: [
                                                                                  ...
                                                                                  Flexible(
                                                                                    child: Column(
                                                                                      ...
                                                                                      children: [
                                                                                        ...
                                                                                        Text(comment['content'], style: const TextStyle(fontSize: 16, color: Colors.black), overflow: TextOverflow.ellipsis, maxLines: 4),
                                                                                        TextButton(
                                                                                          onPressed: () {
                                                                                            setState(() {
                                                                                              parentId = comment['id'];
                                                                                          ...
                                                                                          child: Text(isTranslated ? translatedReplyTextButton : 'Reply', style: TextStyle(color: Colors.grey[600], fontWeight: FontWeight.bold, fontSize: 12)),
                                                                                        ),
                                                                                      ],
                                                                                    ),
                                                                                  )
                                                ...
                                                Padding(
                                                  padding: EdgeInsets.only( bottom: MediaQuery.of(context).viewInsets.bottom  , left: 10.0 , right: 10.0),
                                                  child: Row(
                                                    children: [
                                                      Expanded(
                                                        child: Container(
